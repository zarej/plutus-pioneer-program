...
In a transaction, the sum of the output values must match the sum of
the input values. There are two exceptions.
...
Alice and Bob want to transfer 55 ADA each to Charlie. Alice has only one UTxO, so this
is the UTxO she must use. Bob has two UTxOs, but neither is large enough to cover the 55 ADA he 
wishes to send to Charlie. So, Bob will have to use both his UTxOs as input.
...
The code for the English Auction contract can be found at
...
If all went well with the Plutus setup, you should be able to open the playground at
https://localhost:8009. You will likely receive a certificate error,
which can be bypassed, using whichever method your browser offers you to ignore
such an error.

.. figure:: img/plutus_playground.png
   :alt: Plutus Playground

Copy and paste the EnglishAuction.hs file contents into the playground,
